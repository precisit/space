...
def RocketFunc(w, t, rocket):
	...
	#print vel-np.cross(We, np.array([0,Re,0]))
...
if __name__ == '__main__':
	...
	initial_conds = [initPos[0], initVel[0], initPos[1], initVel[1], initPos[2], initVel[2],
					 402000+90720+17000, 0, 5885.e3]
	time = np.linspace(0,4000,1000) 	
	"""
	Rocket initial conditions
	"""
	R = RocketClass.Rocket(402000., 16000., 3900., 320., 280., 5885.e3, 90720., 3200., 182., 345., 800000., 17000., time[0], 21400, 0.55555556)
	...
	for i in range(len(solutions)):
		...
		tang[:,i] = np.cross(pos[:,i],np.cross(vel[:,i]-np.cross(We, np.array([0,Re,0])),pos[:,i]))
